<div class="container-fluid my-5">
    ...
    <div class="card mt-4">
        ...
        <tr>
            <td colspan="5"></td>
            <td align="right"><span id="total_belanjaan_tabel">Rp. <?= number_format($this->cart->total(), 0, ',', '.') ?></span></td>
        ...
        <br>
        <form method="post" action="<?php echo base_url(). 'transaksi/tambah_data'?> ">
            <div class="row mt-2">
                <div class="col-sm-6">
            ...
            <div class="row">
                ...
                <!-- <div class="row mt-4">
                 ...
                 <div class="row mt-4">
                    <div class="col-sm-12">
                        <input type="hidden" name="total_pesanan" value="">
                        <a href="<?= base_url('welcome/index') ?>" class="btn btn-danger">Kembali ke Beranda</a>
...
<script>
     $(document).ready(function() {
        ...
        success: function(hasil_provinsi) {
            $("select[name=provinsi_pengirim]").empty();
            $("select[name=provinsi_pengirim]").append('<option value="">-- Pilih Provinsi --</option>');

            $.each(hasil_provinsi.rajaongkir.results, function(index, value) {
                $("select[name=provinsi_pengirim]").append('<option value="' + value.province_id + '" id_provinsi=' + value.province_id + '>' + value.province + '</option>');
            });
        }
    });
}); 
 ...
 $("select[name=provinsi_pengirim]").on("change", function() {
        ...
        $.ajax({
            ...
            success: function(hasil_kota) {
                ...
                $("select[name=kota_pengirim]").append('<option value="">-- Pilih Kota --</option>');

                // Tambahkan opsi dropdown dari data JSON
                $.each(hasil_kota.rajaongkir.results, function(index, value) {
                    $("select[name=kota_pengirim]").append('<option value="' + value.city_id + '">' + value.city_name + '</option>');
                });
            }
        });
    });
